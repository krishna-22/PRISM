...
* Python ≥ 3.10
* CUDA-capable GPU (one NVIDIA A100 80GB matches the paper; smaller GPUs work with reduced `--max_tasks_per_env` and 4-bit loading)
...
* `--n_step_labels 60000` — number of prefixes sampled for Monte Carlo labeling (matches the paper)
* `--iql_steps 50000` — number of IQL gradient steps (matches the paper)
* `--prm_epochs 20` — PRM training epochs (matches the paper)
* `--awr_epochs 1` — AWR training epochs (matches the paper)
...
## 5. Hyperparameter reference (matches Table II of the paper)
