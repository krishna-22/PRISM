...
class Trajectory:
    ...
    def transitions(self) -> List[StepTransition]:
        ...
        for t in range(self.length):
            ...
            next_state = self.states[t + 1] if t + 1 < len(self.states) else self.states[-1]
...
class EnvironmentInterface:
    ...
    def _load(self):
        if self.benchmark == "alfworld":
            ...
            try:
                from webshop_lite.envs.web_agent_text_env import WebAgentTextEnv
                self._impl = WebAgentTextEnv
            except Exception:
                self._impl = None
        elif self.benchmark == "scienceworld":
            try:
                from scienceworld import ScienceWorldEnv
                self._impl = ScienceWorldEnv
            except Exception:
                self._impl = None
    ...
    def list_train_tasks(self) -> List[str]:
        counts = {"alfworld": 3553, "webshop": 1000, "scienceworld": 1371}
        return [f"{self.benchmark}_train_{i}" for i in range(counts[self.benchmark])]

    def list_eval_tasks(self) -> List[str]:
        counts = {"alfworld": 134, "webshop": 500, "scienceworld": 270}
        return [f"{self.benchmark}_eval_{i}" for i in range(counts[self.benchmark])]

    def reset(self, task_id: str) -> Tuple[str, str]:
        if self._impl is not None and self.benchmark == "alfworld":
            try:
                env = self._impl.AlfredTWEnv({"env": {"type": "AlfredTWEnv"}}, train_eval="train")
                env = env.init_env(batch_size=1)
                obs, info = env.reset()
                goal = info.get("extra.gamefile", task_id)
                return obs[0] if isinstance(obs, list) else obs, goal
            except Exception:
                pass
        goal = f"Task goal for {task_id}"
        obs = f"Initial observation for {task_id}"
        return obs, goal

    def step(self, action: str, state_idx: int) -> Tuple[str, float, bool, Dict[str, Any]]:
        h = abs(hash(action + str(state_idx))) % 1000
        terminal = (h % 17 == 0)
        reward = 0.0 if not terminal else (1.0 if h % 5 == 0 else 0.0)
        next_obs = f"Observation after action '{action[:40]}' at step {state_idx}"
        return next_obs, reward, terminal, {"raw_reward": reward}
...
class BasePolicy:
    def __init__(self, config: PRISMConfig, lora_adapter_path: Optional[str] = None):
        ...
        if TRANSFORMERS_AVAILABLE:
            self._load_model(lora_adapter_path)

    def _load_model(self, lora_adapter_path: Optional[str]):
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(self.config.base_model_name)
            if self.tokenizer.pad_token is None:
                self.tokenizer.pad_token = self.tokenizer.eos_token
            dtype = torch.bfloat16 if (self.config.bf16 and TORCH_AVAILABLE and torch.cuda.is_available()) else torch.float32
            ...
            if lora_adapter_path is not None and PEFT_AVAILABLE:
    ...
    def sample(self, prompt: str, k: int = 1, temperature: float = 0.7, max_new_tokens: Optional[int] = None) -> List[str]:
        if self.model is None or self.tokenizer is None:
            return [f"Action: stub_action_{i}" for i in range(k)]
...
def rollout_one_trajectory(policy: BasePolicy, env: EnvironmentInterface, task_id: str,
                           goal_text: Optional[str], horizon: int, config: PRISMConfig,
                           reflection: Optional[str] = None) -> Trajectory:
    obs, env_goal = env.reset(task_id)
    goal = goal_text if goal_text is not None else env_goal
    traj = Trajectory(
    ...
    for t in range(horizon):
        ...
        traj.length += 1
        if terminal:
            traj.success = reward >= env.config.success_threshold(env.benchmark) or info.get("success", False)
            traj.final_reward = reward
            break
    if traj.length == 0:
        traj.length = 1
        traj.actions.append("noop")
        traj.rewards.append(0.0)
        traj.states.append(obs)
    return traj


def collect_trajectory_buffer(policy_base: BasePolicy, policy_finetuned: Optional[BasePolicy],
                              envs: Dict[str, EnvironmentInterface], config: PRISMConfig,
    ...
    for benchmark, env in envs.items():
        ...
        for task_id in tasks:
            for _ in range(config.k_collect):
                t = rollout_one_trajectory(policy_base, env, task_id, None, horizon, config)
                if t.length >= 3:
                    buffer.append(t)
            if policy_finetuned is not None:
                for _ in range(config.k_collect_finetuned):
                    t = rollout_one_trajectory(policy_finetuned, env, task_id, None, horizon, config)
                    if t.length >= 3:
                        buffer.append(t)
    seen = set()
    deduped = []
...
def monte_carlo_step_labels(buffer: List[Trajectory], policy: BasePolicy,
                            envs: Dict[str, EnvironmentInterface], config: PRISMConfig) -> List[Dict[str, Any]]:
    triples: List[Dict[str, Any]] = []
    prefix_pool = []
    for tr in buffer:
        for t in range(tr.length):
            prefix_pool.append((tr, t))
    succ = [(tr, t) for (tr, t) in prefix_pool if tr.success]
    fail = [(tr, t) for (tr, t) in prefix_pool if not tr.success]
    ...
    for tr, t in sampled:
        ...
        successes = 0
        for j in range(config.m_continuations):
            obs = tr.states[t] if t < len(tr.states) else tr.states[-1]
            cur_traj = Trajectory(
                trajectory_id=f"{tr.trajectory_id}_cont_{j}",
                benchmark=tr.benchmark,
                task_id=tr.task_id,
                goal_text=tr.goal_text,
                states=list(tr.states[:t + 1]),
                actions=list(tr.actions[:t + 1]),
                rewards=list(tr.rewards[:t + 1]),
                length=t + 1,
            )
            remaining = config.horizon(tr.benchmark) - cur_traj.length
            if remaining <= 0:
                successes += int(tr.success)
                continue
            for step in range(remaining):
                prefix_text = cur_traj.prefix_text(cur_traj.length)
                prompt = policy.build_prompt(tr.benchmark, prefix_text)
                samples = policy.sample(prompt, k=1, temperature=config.decode_temperature)
                action = policy.parse_action(samples[0])
                cur_traj.actions.append(action)
                next_obs, reward, terminal, info = env.step(action, cur_traj.length)
                cur_traj.states.append(next_obs)
                cur_traj.rewards.append(reward)
                cur_traj.length += 1
                if terminal:
                    if reward >= env.config.success_threshold(tr.benchmark) or info.get("success", False):
                        successes += 1
                    break
...
if TORCH_AVAILABLE and TRANSFORMERS_AVAILABLE:
    class EncoderBackbone(nn.Module):
        ...
        def encode(self, texts: List[str], device: torch.device, max_length: int = 512) -> torch.Tensor:
            enc = self.tokenizer(texts, padding=True, truncation=True, max_length=max_length, return_tensors="pt").to(device)
        ...
        for epoch in range(config.prm_epochs):
            ...
            for batch in train_loader:
                ...
                if step % config.log_every == 0:
                    print(f"[PRM] epoch={epoch} step={step} loss={loss.item():.4f}")
            prm.eval()
            with torch.no_grad():
                correct = 0
                total = 0
                ...
                print(f"[PRM] epoch={epoch} val_step_label_acc={acc:.4f}")
        return prm


    def train_confidence_head(conf: ConfidenceHead, buffer: List[Trajectory], config: PRISMConfig, device: torch.device):
        ...
        for epoch in range(config.conf_epochs):
            ...
            for batch in train_loader:
                ...
                step += 1
                if step % config.log_every == 0:
                    print(f"[CONF] epoch={epoch} step={step} loss={loss.item():.4f}")
        conf.eval()
        val_logits = []
        val_labels = []
        with torch.no_grad():
        ...
        print(f"[CONF] ECE_after_platt={ece:.4f} Brier={brier:.4f} platt_a={conf.platt_a:.4f} platt_b={conf.platt_b:.4f}")
        return conf
    ...
    def expected_calibration_error_and_brier(logits: List[float], labels: List[int],
        ...
        for m in range(n_bins):
            ...
            mask = (probs >= lo) & (probs < hi if m < n_bins - 1 else probs <= hi)
            if mask.sum() == 0:
                continue
    ...
    class IQLDataset(Dataset):
        def __init__(self, transitions: List[StepTransition], prm: ProcessRewardModel,
                     trajectories_by_id: Dict[str, Trajectory], config: PRISMConfig, device: torch.device):
            self.transitions = transitions
            self.prm = prm
            self.trajectories = trajectories_by_id
            self.config = config
            self.device = device
        ...
        def __getitem__(self, idx):
            ...
            trajectory = self.trajectories[tr.trajectory_id]
            prefix = trajectory.prefix_text(tr.step_index)
            prefix_action = prefix + "\n[ACT] " + tr.action_text
            with torch.no_grad():
                phi = self.prm.score([prefix_action], self.device).item()
            phi_bar = 2.0 * phi - 1.0
            outcome_term = (1.0 if trajectory.success else 0.0) if tr.terminal else 0.0
            r_hat = self.config.alpha_reward_mix * phi_bar + (1.0 - self.config.alpha_reward_mix) * outcome_term
            next_prefix = trajectory.prefix_text(tr.step_index + 1) if tr.step_index + 1 < trajectory.length else trajectory.prefix_text(tr.step_index)
            return {
                "state_text": prefix,
                "state_action_text": prefix_action,
                "next_state_text": next_prefix,
                "r_hat": float(r_hat),
                "terminal": float(tr.terminal),
            }
    ...
    def expectile_loss(diff: torch.Tensor, tau: float) -> torch.Tensor:
        weight = torch.where(diff < 0, tau - 1.0, tau).abs()
        return (weight * diff.pow(2)).mean()


    def train_iql(value_head: ValueHead, q_head: QValueHead, q_target: QValueHead,
                  backbone: EncoderBackbone, transitions: List[StepTransition],
                  trajectories_by_id: Dict[str, Trajectory], prm: ProcessRewardModel,
                  config: PRISMConfig, device: torch.device):
        ds = IQLDataset(transitions, prm, trajectories_by_id, config, device)
        ...
        while step < total_steps:
            ...
            step += 1
            if step % config.log_every == 0:
                print(f"[IQL] step={step} loss_v={loss_v.item():.4f} loss_q={loss_q.item():.4f}")
        return value_head, q_head


    class AWRSample:
        def __init__(self, prompt: str, action_text: str, advantage: float):
            self.prompt = prompt
    ...
    def build_awr_samples(transitions: List[StepTransition], trajectories_by_id: Dict[str, Trajectory],
        ...
        with torch.no_grad():
            ...
            while i < len(transitions):
                batch = transitions[i:i + config.iql_batch_size]
                state_texts = []
                sa_texts = []
                prompts = []
                actions = []
    ...
    def train_awr_policy(policy: BasePolicy, samples: List[AWRSample], config: PRISMConfig, device: torch.device):
        ...
        total_steps = max(1, config.awr_epochs * (len(samples) // max(config.awr_batch_size, 1)))
        ...
        for epoch in range(config.awr_epochs):
            ...
            for i in range(0, len(samples), config.awr_batch_size):
                batch = samples[i:i + config.awr_batch_size]
                total_loss = torch.tensor(0.0, device=model.device)
                for s in batch:
                    weight_raw = math.exp(min(beta * s.advantage, beta * delta_max))
                    weight = weight_raw if s.advantage >= 0 else weight_raw * config.awr_negative_advantage_weight
                    full_text = s.prompt + " " + s.action_text
                    enc = tokenizer(full_text, return_tensors="pt", truncation=True, max_length=2048).to(model.device)
                    prompt_enc = tokenizer(s.prompt, return_tensors="pt", truncation=True, max_length=2048).to(model.device)
                    prompt_len = prompt_enc["input_ids"].shape[1]
                    labels = enc["input_ids"].clone()
                    labels[:, :prompt_len] = -100
                    out = model(**enc, labels=labels)
                    total_loss = total_loss + weight * out.loss
                ...
                step += 1
                if step % config.log_every == 0:
                    print(f"[AWR] epoch={epoch} step={step} loss={loss.item():.4f}")
        model.eval()
        return policy


    def self_correcting_inference(policy: BasePolicy, prm: ProcessRewardModel, conf: ConfidenceHead,
                                  env: EnvironmentInterface, task_id: str, config: PRISMConfig,
        ...
        for t in range(horizon):
            ...
            for retry in range(config.r_max + 1):
                ...
                with torch.no_grad():
                    c_t = conf.confidence_calibrated([conf_text], device).cpu().numpy().tolist()[0]
            ...
            next_obs, reward, terminal, info = env.step(chosen_action, t)
            traj.rewards.append(reward)
            traj.states.append(next_obs)
            traj.length += 1
            if terminal:
...
def evaluate(policy: BasePolicy, prm: "ProcessRewardModel", conf: "ConfidenceHead",
             envs: Dict[str, EnvironmentInterface], config: PRISMConfig, device: "torch.device",
    ...
    for benchmark, env in envs.items():
            ...
            for task_id in eval_tasks:
                ...
                if benchmark == "alfworld":
                    ...
                    if traj.final_reward >= config.scienceworld_success_threshold:
                        ...
                        steps_used.append(traj.length)
                    scores.append(traj.final_reward)
            per_seed.append(successes / max(len(eval_tasks), 1))
        ...
        results[benchmark] = {
            "success_rate_mean": float(np.mean(per_seed)),
            "success_rate_std": float(np.std(per_seed)),
...
def run_pipeline(config: PRISMConfig, args: argparse.Namespace):
    ...
    if not (TORCH_AVAILABLE and TRANSFORMERS_AVAILABLE):
        print("torch/transformers not available; install requirements before running training.")
        return

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    envs = {b: EnvironmentInterface(b, config) for b in config.benchmarks}

    print("=== Stage A: trajectory collection (base policy) ===")
    base_policy = BasePolicy(config)
    buffer = collect_trajectory_buffer(base_policy, None, envs, config,
                                       max_tasks_per_env=args.max_tasks_per_env)
    print(f"Collected {len(buffer)} base-policy trajectories")

    print("=== Stage A: SFT on successful subset for π1 ===")
    successful = [t for t in buffer if t.success]
    if len(successful) > 0 and args.run_sft:
        sft_samples = []
    ...
    buffer_extra = collect_trajectory_buffer(finetuned_policy, None, envs, config,
                                             max_tasks_per_env=args.max_tasks_per_env)
    buffer.extend(buffer_extra)
    print(f"Total buffer after π1 rollouts: {len(buffer)} trajectories")

    print("=== Stage B1: Monte Carlo step labels ===")
    ...
    with open(os.path.join(config.output_dir, "step_labels.json"), "w") as f:
        json.dump(triples, f)
    print(f"Generated {len(triples)} step-label triples")

    print("=== Stage B2: train PRM and Confidence head ===")
    backbone = EncoderBackbone(config.encoder_model_name)
    ...
    print("=== Stage B3: train IQL critics ===")
    ...
    train_iql(value_head, q_head, q_target, backbone, transitions, trajectories_by_id, prm, config, device)

    print("=== Stage B4: AWR policy extraction over LoRA ===")
